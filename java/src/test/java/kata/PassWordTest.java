...
//	        Assertions.assertTrue("az".matches("/^([a-zA-Z]+)$/"));
    ...
    @Nested
    class TestPasswordForAdmin {
    	@Test
        void pass_word_valid_for_admin(){
        	Password password = new Password("Ab12cdeéà(°£f");
        	
        	Assertions.assertTrue(password.isValidPasswordForAdmin());
        }
        @Test
        void pass_word_not_valid_for_admin(){
        	Password password = new Password("Ab12cdf");
        	
        	Assertions.assertFalse(password.isValidPasswordForAdmin());
        }
        @Test
     ...
     void testDuPO() throws Exception {
         ...
         Assertions.assertTrue(new Password("12àé3456789").isValidPasswordForAdmin());
         Assertions.assertFalse(new Password("HELLO00").isValidPasswordForAdmin());
     }
 }
